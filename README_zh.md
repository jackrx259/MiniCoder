...
<p align="center">
  <img src="assets/demo.webp" alt="MiniCoder 演示">
</p>
...
### 浏览器自动化（`/claw`）
- **Web 智能体模式** — 输入 `/claw <任务>` 进入浏览器智能体模式。MiniCoder 启动 Chromium 浏览器并自动执行网页交互：导航、点击、输入、截图等。
- **人机协同** — 浏览器操作使用与其他工具相同的计划审批流程。每一步都会先展示给你确认。
- **基于 Playwright** — 可靠的有界面浏览器自动化，支持自动等待和智能选择器。
...
# 安装 Chromium 用于浏览器自动化（可选，/claw 命令需要）
...
| `browser_navigate` | 导航到新 URL |
| `browser_click` | 点击元素（CSS 选择器或文本） |
| `browser_type` | 在输入框中输入文本 |
| `browser_screenshot` | 截取当前页面截图 |
| `browser_get_text` | 获取页面可见文本内容 |
| `browser_get_elements` | 列出页面上的可交互元素 |
